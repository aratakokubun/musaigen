...
uniform sampler2D alphabets;
uniform sampler2D alphaMatrixTexture;
...
// 分割数
const vec2 div = vec2(64.0, 64.0);
const vec2 divRange = vec2(mag.x/div.x, mag.y/div.y);
const float nFrag = 1.0 / (divRange.x*divRange.y);
// Mode
const int MUSAIGEN = 0;
const int HSVFILTER = 1;
const int MOSAIC = 2;
const int ORIGINAL = 3;

vec4 getAlphabetTexture2D(vec2 magUv, vec2 posInAlphabet) {
  ...
  return texture2D(alphabets, alphabetCoords);
}
...
vec4 applyEffect(vec2 uvCoord) {
  ...
  if (hsvFilter(aveColor.rgb)) {
    return getAlphabetTexture2D(magUv, vec2(offset.x/divRange.x, offset.y/divRange.y));
  } else {
    return vec4(0.0);
  }
}
