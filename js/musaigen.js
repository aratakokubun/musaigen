// standard global variables
var container, scene, camera, renderer, controls, stats;
var keyboard = new THREEx.KeyboardState();
var clock = new THREE.Clock();
var myShader;
var myShaderUniforms;

// Shader material
var musaigenVertexShader;
var musaigeFragmentShader;
var hsvFilterFragmentShader;
var mosaicFragmentShader;
var simpleFragmentShader;

// Skip frames
const skipFrames = 10;
// Frame count to update view
var frameCount = 0;

// Texture division number (r, c)
var div = new THREE.Vector2(64.0, 64.0);
// Texture block size
var mag = new THREE.Vector2(512.0, 512.0);
var divRange = new THREE.Vector2(mag.x/div.x, mag.y/div.y);
var nFrag = 1.0 / (divRange.x*divRange.y);

// Array to specify alphabet indices
var alphaMatrix = new Array(div.x*div.y);
// Array to convert alphabet indices to RGBA texture
var alphaUint8Matrix = new Uint8Array(div.x*div.y*4/*rgba*/);
// Alphabet offset corrd array
var alphabetOffsetArray = [new THREE.Vector2( 0.0, 0.6 ), // A
                           new THREE.Vector2( 0.1, 0.6 ), // B
                           new THREE.Vector2( 0.2, 0.6 ), // C
                           new THREE.Vector2( 0.3, 0.6 ), // D
                           new THREE.Vector2( 0.4, 0.6 ), // E
                           new THREE.Vector2( 0.5, 0.6 ), // F
                           new THREE.Vector2( 0.6, 0.6 ), // G
                           new THREE.Vector2( 0.7, 0.6 ), // H
                           new THREE.Vector2( 0.8, 0.6 ), // I
                           new THREE.Vector2( 0.9, 0.6 ), // J
                           new THREE.Vector2( 0.0, 0.4 ), // K
                           new THREE.Vector2( 0.1, 0.4 ), // L
                           new THREE.Vector2( 0.2, 0.4 ), // M
                           new THREE.Vector2( 0.3, 0.4 ), // N
                           new THREE.Vector2( 0.4, 0.4 ), // O
                           new THREE.Vector2( 0.5, 0.4 ), // P
                           new THREE.Vector2( 0.6, 0.4 ), // Q
                           new THREE.Vector2( 0.7, 0.4 ), // R
                           new THREE.Vector2( 0.8, 0.4 ), // S
                           new THREE.Vector2( 0.9, 0.4 ), // T
                           new THREE.Vector2( 0.0, 0.2 ), // U
                           new THREE.Vector2( 0.1, 0.2 ), // V
                           new THREE.Vector2( 0.2, 0.2 ), // W
                           new THREE.Vector2( 0.3, 0.2 ), // X
                           new THREE.Vector2( 0.4, 0.2 ), // Y
                           new THREE.Vector2( 0.5, 0.2 ), // Z
                           new THREE.Vector2( 0.0, 0.0 ), // 1
                           new THREE.Vector2( 0.1, 0.0 ), // 2
                           new THREE.Vector2( 0.2, 0.0 ), // 3
                           new THREE.Vector2( 0.3, 0.0 ), // 4
                           new THREE.Vector2( 0.4, 0.0 ), // 5
                           new THREE.Vector2( 0.5, 0.0 ), // 6
                           new THREE.Vector2( 0.6, 0.0 ), // 7
                           new THREE.Vector2( 0.7, 0.0 ), // 8
                           new THREE.Vector2( 0.8, 0.0 ), // 9
                           new THREE.Vector2( 0.9, 0.0 )]; // 0
var alphaSize = new THREE.Vector2( 0.1, 0.2 );

// Get camera video
// Put event listeners into place
var videoTexture;
var video = document.createElement('video');
var videoWidth=256, videoHeight=256;

// Image texture
var imageTexture;
var imageWidth = 256, imageHeight = 256;
// Alphabet texture
var alphabetTexture;
// Texture Geometry
var textureGeometry;

// GUI
const MIN_H = 340.0;
const MIN_S = 20.0;
const MIN_V = 50.0;
const MAX_H = 20.0;
const MAX_S = 180.0;
const MAX_V = 180.0;
var HsvCtrl = function() {
  this.minH = MIN_H;
  this.minS = MIN_S;
  this.minV = MIN_V;
  this.maxH = MAX_H;
  this.maxS = MAX_S;
  this.maxV = MAX_V;
};
const MUSAIGEN = 0;
const HSVFILTER = 1;
const MOSAIC = 2;
const ORIGINAL = 3;
var ModeCheckBox = function() {
  this.isMusaingen = true;
  this.isHSVFilter = false;
  this.isMosaic = false;
  this.isOriginal = false;
  this.modeId = MUSAIGEN;
};
const VIDEO = 0;
const IMAGE = 1;
var TextureCheckBox = function() {
  this.sampleImageTexture = false;
  this.videoTexture = true;
};

var gui = new dat.GUI();
var hsvCtrlObj = new HsvCtrl();
var modeCheckBox = new ModeCheckBox();
var textureModeCheckBox = new TextureCheckBox();
var folder = gui.addFolder('Musaigen Mosaic Control');
// Seek bars
folder.add(hsvCtrlObj, 'minH', 0, 360).onChange(updateHSVThreashold);
folder.add(hsvCtrlObj, 'minS', 0, 180).onChange(updateHSVThreashold);
folder.add(hsvCtrlObj, 'minV', 0, 180).onChange(updateHSVThreashold);
folder.add(hsvCtrlObj, 'maxH', 0, 360).onChange(updateHSVThreashold);
folder.add(hsvCtrlObj, 'maxS', 0, 180).onChange(updateHSVThreashold);
folder.add(hsvCtrlObj, 'maxV', 0, 180).onChange(updateHSVThreashold);
// Mode radio buttons
folder.add(modeCheckBox, "isMusaingen").onChange(updateCheckboxMusaigen).listen();
folder.add(modeCheckBox, "isHSVFilter").onChange(updateCheckboxHSVFilter).listen();
folder.add(modeCheckBox, "isMosaic").onChange(updateCheckboxMosaic).listen();
folder.add(modeCheckBox, "isOriginal").onChange(updateCheckboxOriginal).listen();
// Texture mode radio buttons
folder.add(textureModeCheckBox, "sampleImageTexture").onChange(updateTextureImage).listen();
folder.add(textureModeCheckBox, "videoTexture").onChange(updateTextureVideo).listen();

// MAIN
loadVideoCamera();
preload();

// FUNCTIONS
function preload()
{
  // Preload of shader files
  SHADER_LOADER.load(
    // Callback function on complete load
    function (data)
    {
      // Vertex shaders
      musaigenVertexShader = data.musaigenShader.vertex;
      // Fragment shaders
      musaigeFragmentShader = data.musaigenShader.fragment;
      hsvFilterFragmentShader = data.hsvFilterShader.fragment;
      mosaicFragmentShader = data.mosaicShader.fragment;
      simpleFragmentShader = data.simpleShader.fragment;

      // Initialize three js
      init();
      clock.start();
      animate();
    }
  );
}

function init() 
{
  // SCENE
  scene = new THREE.Scene();
  // CAMERA
  var SCREEN_WIDTH = window.innerWidth, SCREEN_HEIGHT = window.innerHeight;
  var VIEW_ANGLE = 45, ASPECT = SCREEN_WIDTH / SCREEN_HEIGHT, NEAR = 0.1, FAR = 20000;
  camera = new THREE.PerspectiveCamera( VIEW_ANGLE, ASPECT, NEAR, FAR);
  scene.add(camera);
  camera.position.set(0,0,400);
  camera.lookAt(scene.position);  
  // RENDERER
  if ( Detector.webgl )
    renderer = new THREE.WebGLRenderer( {antialias:true} );
  else
    renderer = new THREE.CanvasRenderer(); 
  renderer.setSize(SCREEN_WIDTH, SCREEN_HEIGHT);
  container = document.getElementById( 'ThreeJS' );
  container.appendChild( renderer.domElement );
  // EVENTS
  THREEx.WindowResize(renderer, camera);
  THREEx.FullScreen.bindKey({ charCode : 'm'.charCodeAt(0) });
  // CONTROLS
  controls = new THREE.OrbitControls( camera, renderer.domElement );
  // STATS
  stats = new Stats();
  stats.domElement.style.position = 'absolute';
  stats.domElement.style.bottom = '0px';
  stats.domElement.style.zIndex = 100;
  container.appendChild( stats.domElement );
  // LIGHT
  var light = new THREE.PointLight(0xffffff);
  light.position.set(0,150,100);
  scene.add(light);
  
  var light2 = new THREE.AmbientLight(0x444444);
  scene.add(light2);

  // Texture Image
  loadImage('image/texture.png');

  // Alphabet Image
  alphabetTexture = THREE.ImageUtils.loadTexture('image/alphabet2.png');
  alphabetTexture.magFilter = THREE.NearestFilter;
  alphabetTexture.minFilter = THREE.NearestFilter;

  // Create alphabet offset texture to specify which alphabet to be drawn in each cell
  // https://github.com/mrdoob/three.js/issues/1235
  initAlphabetMatrix();
  createAlphabetMatrixTexture();

  // Musaigen Effect Shader
  myShaderUniforms = {
    // texture: { type: 't', value: imageTexture},
    texture: { type: 't', value: videoTexture},
    alphabets: { type: 't', value: alphabetTexture},
    alphaMatrixTexture: { type: 't', value: alphaMatrixTexture}, 
    div: { type: 'v2', value: div},
    mag: { type: 'v2', value: mag},
    divRange: { type: 'v2', value: divRange},
    nFrag: { type: 'f', value: nFrag},
    minThreashold : { type: 'v3', value: new THREE.Vector3( MIN_H, MIN_S, MIN_V ) },
    maxThreashold : { type: 'v3', value: new THREE.Vector3( MAX_H, MAX_S, MAX_V ) }
  };
  updateShader(MUSAIGEN);
}

function loadVideoCamera()
{
  window.addEventListener("DOMContentLoaded", function() {
    // Grab elements, create settings, etc.
    var videoObj = { "video": true },
      errBack = function(error) {
        console.log("Video capture error: ", error.code); 
      };

    // Put video listeners into place
    if(navigator.getUserMedia) { // Standard
      navigator.getUserMedia(videoObj, function(stream) {
        video.src = stream;
        video.play();
        videoTexture = new THREE.VideoTexture( video );
        videoTexture.magFilter = THREE.NearestFilter;
        videoTexture.minFilter = THREE.NearestFilter;
      }, errBack);
    } else if(navigator.webkitGetUserMedia) { // WebKit-prefixed
      navigator.webkitGetUserMedia(videoObj, function(stream){
        video.src = window.webkitURL.createObjectURL(stream);
        video.play();
        videoTexture = new THREE.VideoTexture( video );
        videoTexture.magFilter = THREE.NearestFilter;
        videoTexture.minFilter = THREE.NearestFilter;
      }, errBack);
    }
    else if(navigator.mozGetUserMedia) { // Firefox-prefixed
      navigator.mozGetUserMedia(videoObj, function(stream){
        video.src = window.URL.createObjectURL(stream);
        video.play();
        videoTexture = new THREE.VideoTexture( video );
        videoTexture.magFilter = THREE.NearestFilter;
        videoTexture.minFilter = THREE.NearestFilter;
      }, errBack);
    }
    else {
        console.log("failed to get media");
        return;
    }

    video.addEventListener( "loadedmetadata", function (e) {
      // Scale 1/2 beacuset to large to display in default clip
      videoWidth = this.videoWidth / 2,
      videoHeight = this.videoHeight / 2;
      // Reload shader on reload completely
      updateShader();
    }, false );
  }, false);
}

function loadImage(showImgSrc)
{
    var newImg = new Image();

    newImg.onload = function() {
      imageWidth = newImg.width;
      imageHeight = newImg.height;
      imageTexture = THREE.ImageUtils.loadTexture(showImgSrc);
      imageTexture.magFilter = THREE.NearestFilter;
      imageTexture.minFilter = THREE.NearestFilter;
    }
    newImg.src = showImgSrc; // this must be done AFTER setting onload
}

function initAlphabetMatrix()
{
  for (var i=0; i < div.x*div.y; i++) {
    alphaMatrix[i] = Math.floor( Math.random() * alphabetOffsetArray.length );
  }
}

function createAlphabetMatrixTexture() {
  // Set (x,y,w,h) -> (r, g, b, a)
  for (var i=0; i < div.x*div.y; i++) {
    // matrix index -> coords(x,y,w,h)
    var offset = alphabetOffsetArray[alphaMatrix[i]];
    alphaUint8Matrix[i*4  ] = Math.floor(offset.x * 255);
    alphaUint8Matrix[i*4+1] = Math.floor(offset.y * 255);
    alphaUint8Matrix[i*4+2] = Math.floor(alphaSize.x * 255);
    alphaUint8Matrix[i*4+3] = Math.floor(alphaSize.y * 255);
  }
  alphaMatrixTexture = new THREE.DataTexture(alphaUint8Matrix, div.x, div.y, THREE.RGBAFormat);
  alphaMatrixTexture.needsUpdate = true;
  alphaMatrixTexture.magFilter = THREE.NearestFilter;
  alphaMatrixTexture.minFilter = THREE.NearestFilter;
}

function updateAlphabetMatrix()
{
  // Shift 1 row from last to 1 row
  for (var i=0; i < div.y-1; i++) {
    for (var j=0; j < div.x; j++) {
      alphaMatrix[i*div.x+j] = alphaMatrix[(i+1)*div.x+j];
    }
  }
  // Create random row at 0 row
  for (var i=0; i < div.x; i++) {
    alphaMatrix[(div.y-1)*div.x+i] = Math.floor( Math.random() * alphabetOffsetArray.length );
  }
  createAlphabetMatrixTexture();
  myShaderUniforms.alphaMatrixTexture.value = alphaMatrixTexture;
}

function updateTextureDivision()
{
  divRange = new THREE.Vector2(mag.x/div.x, mag.y/div.y);
  nFrag = 1.0 / (divRange.x*divRange.y);
}

function updateShader()
{
  // Remove mesh
  scene.remove( myShader );
  // textureGeometry.dispose();

  // Switch shader
  var fragment;
  switch (modeCheckBox.modeId) {
    case MUSAIGEN:
      fragment = musaigeFragmentShader;
      break;
    case HSVFILTER:
      fragment = hsvFilterFragmentShader;
      break;
    case MOSAIC:
      fragment = mosaicFragmentShader;
      break;
    case ORIGINAL:
      fragment = simpleFragmentShader;
      break;
  }

  // Get image size
  if (textureModeCheckBox.videoTexture) {
    textureGeometry = new THREE.PlaneGeometry(videoWidth, videoHeight, 1, 1);
    myShaderUniforms.texture.value = videoTexture;
  } else {
    textureGeometry = new THREE.PlaneGeometry(imageWidth, imageHeight, 1, 1);
    myShaderUniforms.texture.value = imageTexture;
  }

  var shaderMaterial = new THREE.ShaderMaterial({
    vertexShader: musaigenVertexShader,
    fragmentShader: fragment,
    uniforms: myShaderUniforms
  });
  shaderMaterial.side = THREE.DoubleSide;
  shaderMaterial.transparent = true;
  shaderMaterial.blending = THREE.NormalBlending;
  myShader = new THREE.Mesh(textureGeometry, shaderMaterial);
  scene.add( myShader );
}

function animate() 
{
  update();
  render();   
  requestAnimationFrame( animate );
}

function update()
{
  // Update frame
  frameCount+= 1;

  if (frameCount >= skipFrames) {
    updateAlphabetMatrix();
    updateTextureDivision();
    frameCount = 0;
  }
  controls.update();
  stats.update();
}

function render() 
{
  renderer.render( scene, camera );
}

// GUI CONTTROLS
function updateHSVThreashold()
{
  myShaderUniforms.minThreashold.value 
    = new THREE.Vector3(hsvCtrlObj.minH, hsvCtrlObj.minS, hsvCtrlObj.minV);
  myShaderUniforms.maxThreashold.value 
    = new THREE.Vector3(hsvCtrlObj.maxH, hsvCtrlObj.maxS, hsvCtrlObj.maxV);
}

function updateCheckboxMusaigen()
{
  modeCheckBox.isMusaingen = true;
  modeCheckBox.isHSVFilter = false;
  modeCheckBox.isMosaic = false;
  modeCheckBox.isOriginal = false;
  modeCheckBox.modeId = MUSAIGEN;
  updateShader();
}
function updateCheckboxHSVFilter()
{
  modeCheckBox.isMusaingen = false;
  modeCheckBox.isHSVFilter = true;
  modeCheckBox.isMosaic = false;
  modeCheckBox.isOriginal = false;
  modeCheckBox.modeId = HSVFILTER;
  updateShader();
}
function updateCheckboxMosaic()
{
  modeCheckBox.isMusaingen = false;
  modeCheckBox.isHSVFilter = false;
  modeCheckBox.isMosaic = true;
  modeCheckBox.isOriginal = false;
  modeCheckBox.modeId = MOSAIC;
  updateShader();
}
function updateCheckboxOriginal()
{
  modeCheckBox.isMusaingen = false;
  modeCheckBox.isHSVFilter = false;
  modeCheckBox.isMosaic = false;
  modeCheckBox.isOriginal = true;
  modeCheckBox.modeId = ORIGINAL;
  updateShader();
}

function updateTextureImage()
{
  textureModeCheckBox.sampleImageTexture = true;
  textureModeCheckBox.videoTexture = false;
  updateShader();
}

function updateTextureVideo()
{
  textureModeCheckBox.sampleImageTexture = false;
  textureModeCheckBox.videoTexture = true;
  updateShader();
}
